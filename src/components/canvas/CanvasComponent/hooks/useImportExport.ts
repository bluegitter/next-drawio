...
export const useImportExport = ({
  ...
  const parsePointsString = useCallback((points: string) => {
    return points
      .trim()
      .split(/\s+/)
      .map(pair => {
  ...
  const getPointsArray = useCallback((points: any): Array<{ x: number; y: number }> => {
    if (!points) return [];
  ...
  const applyCommonAttributes = useCallback((el: SVGElement, data: any) => {
  ...
  const applyTransformFromData = useCallback((el: SVGElement, type: string, data: any) => {
  ...
  const buildElementFromData = useCallback((type: string, data: any) => {
    if (!data) return null;
    ...
    return null;
  }, [applyCommonAttributes, applyTransformFromData, createSVGElement, getPointsArray]);
