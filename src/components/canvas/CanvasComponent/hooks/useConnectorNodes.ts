...
interface UseConnectorNodesArgs {
  shapes: SVGShape[];
...
export const useConnectorNodes = ({
  ...
  const addConnectorNodeAt = useCallback((shape: SVGShape, clientX: number, clientY: number) => {
  ...
  }, [getConnectorPoints, getPointerPosition, onShapeSelect, projectPointToSegment, saveToHistory, selectedIds, setSelectedShape, setShapesState, shapes, updateConnectorPoints]);
