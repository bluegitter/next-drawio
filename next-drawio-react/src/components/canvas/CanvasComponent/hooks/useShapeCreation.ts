...
import { decodeDataUri, toDataUri } from '@drawio/core';
import { formatPoints } from '@drawio/core';

interface UseShapeCreationArgs {
...
export const useShapeCreation = ({
  ...
  }, [createSVGElement, generateId, getDef, getShapeBounds, onShapeSelect, saveToHistory, setSelectedShape, setShapesState, svgRef]);
  ...
  const connectShapes = useCallback((fromShape: string, toShape: string, fromPortId?: string, toPortId?: string) => {
    ...
    const pointsString = formatPoints([[fromPoint.x, fromPoint.y], [toPoint.x, toPoint.y]]);
  ...
  }, [createSVGElement, formatPoints, generateId, getPortPositionById, getShapeCenter, saveToHistory, selectedIds, shapes, setConnectionStart, setConnectionStartPort, setIsConnecting, setShapesState, setTempLine, svgRef, tempLine]);
