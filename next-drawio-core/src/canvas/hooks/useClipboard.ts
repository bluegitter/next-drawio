...
export const useClipboard = ({
  ...
  const duplicateSelected = (ids?: Set<string> | string[], sourceShapesOverride?: SVGShape[]) => {
    ...
    const targetIds = ids ? (ids instanceof Set ? ids : new Set(ids)) : selected;
    if (targetIds.size === 0 || !svg) return;

    const sourceShapes = sourceShapesOverride ?? (getRefValue(shapes) ?? []);
    const offset = 20;
    ...
    sourceShapes.forEach((sourceShape) => {
      ...
      svg.appendChild(duplicatedShape.element);
    ...
    const mergedShapes = sourceShapesOverride
      ? [...(getRefValue(shapes) ?? []), ...newShapes]
      : [...sourceShapes, ...newShapes];
    setShapesState(() => mergedShapes);
    const newIds = newShapes.map((s) => s.id);
    setSelectedIds(new Set(newIds));
    setRefValue(copyBufferRef, { ids: newIds, shapes: newShapes.map(cloneShapeForClipboard) });
    onClipboardChange?.((getRefValue(copyBufferRef)?.ids.length || 0) > 0);
  ...
  const copySelection = () => {
    ...
    const snapshot = buildClipboardSnapshot(selected, shapeList);
    setRefValue(copyBufferRef, snapshot);
    onClipboardChange?.(snapshot.ids.length > 0);
    return snapshot.ids.length;
  };

  const pasteClipboard = () => {
    const buffer = getRefValue(copyBufferRef);
    if (!buffer || buffer.ids.length === 0) return 0;
    ...
    if (result && result.ids.length) {
      ...
      setRefValue(copyBufferRef, snapshot.ids.length ? snapshot : { ids: result.ids, shapes: buffer.shapes });
      onClipboardChange?.((getRefValue(copyBufferRef)?.ids.length || 0) > 0);
  ...
  const hasClipboard = () => {
    return Boolean(getRefValue(copyBufferRef)?.ids.length);
  };
