...
type UseSelectionArgs = {
  ...
  resizeHandlesRef: RefLike<Map<string, SVGElement[]>>;
  cornerHandlesRef: RefLike<Map<string, SVGElement[]>>;
...
export const useSelection = ({
  ...
  const beginEditText = (shape: SVGShape) => {
    ...
    setTimeout(() => getRefValue(editingInputRef)?.focus(), 0);
  };

  const hideResizeHandles = (shapeId?: string) => {
    const resizeHandles = getRefValue(resizeHandlesRef);
    if (!resizeHandles) return;
    if (shapeId) {
      const handles = resizeHandles.get(shapeId);
      handles?.forEach((h) => h.remove());
      resizeHandles.delete(shapeId);
      return;
    }
    resizeHandles.forEach((list) => list.forEach((h) => h.remove()));
    resizeHandles.clear();
  };

  const hideCornerHandles = (shapeId?: string) => {
    const cornerHandles = getRefValue(cornerHandlesRef);
    if (!cornerHandles) return;
    if (shapeId) {
      const handles = cornerHandles.get(shapeId);
      handles?.forEach((h) => h.remove());
      cornerHandles.delete(shapeId);
      return;
    }
    cornerHandles.forEach((list) => list.forEach((h) => h.remove()));
    cornerHandles.clear();
  };
  ...
  const showResizeHandles = (shape: SVGShape) => {
    ...
    points.forEach((p) => {
      ...
      const onDown = (e: MouseEvent) => {
        ...
        setDragStart({
          x: pos.x,
          y: pos.y,
          viewBoxMinX: getRefValue(viewBoxMinX) ?? 0,
          viewBoxMinY: getRefValue(viewBoxMinY) ?? 0,
        });
      };
    ...
    resizeHandles.set(shape.id, created);
  };

  const showCornerHandles = (shape: SVGShape) => {
    const svg = getRefValue(svgRef);
    const cornerHandlesMap = getRefValue(cornerHandlesRef);
    if (!svg || !cornerHandlesMap || shape.type !== 'roundedRect') return;
    
    // 检查是否已经存在handles，避免重复创建
    const existingHandles = cornerHandlesMap.get(shape.id);
    ...
    cornerHandles.forEach((corner) => {
      ...
      getRefValue(svgRef)?.appendChild(handle);
      created.push(handle);
    });

    getRefValue(cornerHandlesRef)?.set(shape.id, created);
  };
  ...
  const refreshResizeHandles = (shape: SVGShape) => {
    const handles = getRefValue(resizeHandlesRef)?.get(shape.id);
  ...
  const hideTextSelection = (shapeId?: string) => {
    const textSelection = getRefValue(textSelectionRef);
    if (!textSelection) return;
    if (shapeId) {
      const handle = textSelection.get(shapeId);
      handle?.remove();
      textSelection.delete(shapeId);
      return;
    }
    textSelection.forEach((h) => h.remove());
    textSelection.clear();
  };
  ...
  const showTextSelection = (shape: SVGShape) => {
    const svg = getRefValue(svgRef);
    const textSelection = getRefValue(textSelectionRef);
    if (!svg || !textSelection || shape.type !== 'text') return;
    ...
    outline.setAttribute('pointer-events', 'none');
    svg.appendChild(outline);
    textSelection.set(shape.id, outline as SVGRectElement);
  };
