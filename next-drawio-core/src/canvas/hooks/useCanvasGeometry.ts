...
export const useCanvasGeometry = ({ svgRef, shapes, shapeIdCounter }: UseCanvasGeometryArgs) => {
  const generateId = () => {
    const next = (getRefValue(shapeIdCounter) ?? 0) + 1;
    if ('current' in shapeIdCounter) {
      shapeIdCounter.current = next;
    } else if ('value' in shapeIdCounter) {
      shapeIdCounter.value = next;
    }
    return `shape-${next}`;
  };
  ...
  const ensureConnectorPolylineElement = (shape: SVGShape, pointsString: string) => {
    ...
    svg.replaceChild(poly, shape.element);
    return poly;
  };
  ...
  const updateConnectionLine = (connLine: SVGShape, shapeId: string, shapeList?: SVGShape[]) => {
    const source = shapeList || getRefValue(shapes) || [];
