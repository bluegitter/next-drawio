...
export type UseConnectorNodesArgs = {
  ...
  projectPointToSegment: (x: number, y: number, x1: number, y1: number, x2: number, y2: number) => {
    x: number;
    y: number;
  };
...
export const useConnectorNodes = ({
  ...
  const addConnectorNodeAt = (shape: SVGShape, clientX: number, clientY: number) => {
    ...
    const updatedShapes = shapeList.map((s) => (s.id === shape.id ? nextShape : s));
    setShapesState(() => updatedShapes);
    setSelectedShape(shape.id);
    onShapeSelect?.(nextShape.element);
    saveToHistory(updatedShapes, getRefValue(selectedIds) ?? new Set());
  };
