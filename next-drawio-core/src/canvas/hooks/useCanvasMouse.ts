import type { SVGShape } from '../types';
import type { MaybeRef, RefLike } from '../../utils/refs';
import { getRefValue, setRefValue } from '../../utils/refs';

export type UseCanvasMouseArgs = {
  svgRef: RefLike<SVGSVGElement>;
  shapes: MaybeRef<SVGShape[]>;
  selectedShape: MaybeRef<string | null>;
  isConnecting: MaybeRef<boolean>;
  tempLine: MaybeRef<SVGElement | null>;
  skipNextCanvasClickClear: RefLike<boolean>;
  disableSelectionBox?: boolean;
  getPointerPosition: (clientX: number, clientY: number) => { x: number; y: number };
  getConnectorPoints: (shape: SVGShape) => Array<[number, number]>;
  pointToPolylineDistance: (x: number, y: number, points: Array<[number, number]>) => number;
  setIsSelectingBox: (next: boolean) => void;
  setSelectionRect: (next: { x: number; y: number; w: number; h: number } | null) => void;
  setSelectedShape: (id: string | null) => void;
  setIsConnecting: (next: boolean) => void;
  setConnectionStart: (next: string | null) => void;
  setConnectionStartPort: (next: string | null) => void;
  setTempLine: (next: SVGElement | null) => void;
  selectionOriginRef: RefLike<{ x: number; y: number } | null>;
  onShapeSelect?: (shape: SVGElement | null) => void;
};

export const useCanvasMouse = ({
  svgRef,
  shapes,
  selectedShape,
  isConnecting,
  tempLine,
  skipNextCanvasClickClear,
  disableSelectionBox = false,
  getPointerPosition,
  getConnectorPoints,
  pointToPolylineDistance,
  setIsSelectingBox,
  setSelectionRect,
  setSelectedShape,
  setIsConnecting,
  setConnectionStart,
  setConnectionStartPort,
  setTempLine,
  selectionOriginRef,
  onShapeSelect,
}: UseCanvasMouseArgs) => {
  const handleCanvasMouseDown = (e: MouseEvent) => {
    if (disableSelectionBox) return;
    if (e.target === getRefValue(svgRef)) {
      const { x, y } = getPointerPosition(e.clientX, e.clientY);
      setIsSelectingBox(true);
      setSelectionRect({ x, y, w: 0, h: 0 });
      setRefValue(selectionOriginRef, { x, y });
      setSelectedShape(null);
      onShapeSelect?.(null);
    }
  };

  const handleCanvasClick = (e: MouseEvent) => {
    const svg = getRefValue(svgRef);
    if (e.target === svg) {
      const skipNext = Boolean(getRefValue(skipNextCanvasClickClear));
      if (skipNext) {
        setRefValue(skipNextCanvasClickClear, false);
        return;
      }
      let shouldClear = true;

      const selectedShapeValue = getRefValue(selectedShape);
      const shapeList = getRefValue(shapes) ?? [];
      if (selectedShapeValue) {
        const current = shapeList.find((s) => s.id === selectedShapeValue);
        if (current && (current.type === 'line' || current.type === 'connector') && svg) {
          const { x, y } = getPointerPosition(e.clientX, e.clientY);
          const points: [number, number][] =
            current.type === 'connector'
              ? getConnectorPoints(current)
              : [
                  [current.data.x1 || 0, current.data.y1 || 0],
                  [current.data.x2 || 0, current.data.y2 || 0],
                ];
          const dist = pointToPolylineDistance(x, y, points);
          const tolerance = 8;
          if (dist <= tolerance) {
            shouldClear = false;
            setSelectedShape(current.id);
            onShapeSelect?.(current.element);
          }
        }
      }

      if (shouldClear) {
        setSelectedShape(null);
        onShapeSelect?.(null);
      }

      const isConnectingValue = Boolean(getRefValue(isConnecting));
      const tempLineValue = getRefValue(tempLine);
      if (isConnectingValue && tempLineValue && svg) {
        svg.removeChild(tempLineValue);
        setTempLine(null);
      }
      setIsConnecting(false);
      setConnectionStart(null);
      setConnectionStartPort(null);
    }
  };

  return { handleCanvasMouseDown, handleCanvasClick };
};
